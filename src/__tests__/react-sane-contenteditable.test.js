...
describe('Handles props', () => {
  ...
  it('props.caretPosition sets selection on mount', () => {
    const wrapper = mount(<ContentEditable content="foo" caretPosition="start" />);

    expect(wrapper.state('caretPosition')).toEqual(0);
  });

  it('props.caretPosition sets selection on update', () => {
    const wrapper = mount(<ContentEditable content="foo" caretPosition="end" />);
...
describe('Calls handlers', () => {
  ...
  it('props.onKeyDown called', () => {
    ...
    wrapper.childAt(0).simulate('keydown', { which: 13, target: { innerText: content } });
